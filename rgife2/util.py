...
def write_summary(path_results, metric, best_performance):
    ...
    with open(path_results + "summary.txt", "wt") as output:
        output.write("RGIFE selected {0} attributes that provide {1} = {2}\n".format(
            len(selected_attributes), metric, best_performance))
...
def copy_final_results(path_results, best_iteration, reference_iteration, pre_processing, name, binarised_attribute_mapping, dataset_name):
    ...
    if pre_processing == 0:
        args = ["cp", path_results + "it"+str(best_iteration)+"/"+name+"_it"+str(
            best_iteration)+".arff", path_results + "BestIteration/selected_best_data.arff"]
        subprocess.call(args)
        args = ["cp", path_results + "it"+str(reference_iteration)+"/"+name+"_it"+str(
            reference_iteration)+".arff", path_results + "ReferenceIteration/selected_reference_data.arff"]
        subprocess.call(args)
        return
...
def parse_configuration(configuration_file):
    ...
    if config.has_option("parameters", 'block_type'):
        ...
        if block_type not in good_values:
            print ("Invalid block_type: {0} ({1})".format(
                block_type, good_values))
            sys.exit(1)

    if config.has_option("parameters", 'validation'):
        validation_schema = config.get("parameters", "validation").upper()
        good_values = ["6CV", "10CV", "LOOCV"]  # 8899 + "6CV"
        if validation_schema not in good_values:
            print ("Invalid validation: {0} ({1})".format(
                validation_schema, good_values))
            sys.exit(1)
    ...
    if config.has_option("parameters", 'missing_values'):
        ...
        if missing_values not in good_values:
            print ("Invalid missing_values: {0} ({1})".format(
                missing_values, good_values))
            sys.exit(1)

    if config.has_option("parameters", 'categorical_attributes'):
        categorical_attributes = config.get(
            "parameters", "categorical_attributes")
        good_values = ["yes", "no"]
        if categorical_attributes not in good_values:
            print ("Invalid categorical_attributes: {0} ({1})".format(
                categorical_attributes, good_values))
            sys.exit(1)

    if config.has_option("parameters", 'cs_rf'):
        cs_rf = config.get("parameters", "cs_rf").lower()
        good_values = ["auto","manual","no"]
        if cs_rf not in good_values:
            ...
            if cs_rf == "manual":
                cost = config.get(
                    "parameters", "misclassification_cost").split(",")
    ...
    if config.has_option("parameters", 'metric'):
        ...
        good_values = ["accuracy", "robust_accuracy",
                       "fscore", "gmean", "auc", "overall_auc"]
    ...
    if config.has_option("parameters", 'different_folds'):
        ...
        if different_folds not in good_values:
            print ("Invalid different_folds: {0} ({1})".format(
                different_folds, good_values))
            sys.exit(1)
    ...
    if config.has_option("parameters", 'tolerance_samples'):
        ...
        try:
            tolerance_samples = float(config.get(
                "parameters", "tolerance_samples"))
        except ValueError:
        ...
        if "." in tolerance_samples:
            ...
            tolerance_samples = int(config.get(
                "parameters", "tolerance_samples"))
    ...
    if config.has_option("parameters", 'cv_schema'):
        ...
        if cv_schema not in good_values:
            print ("Invalid cv_schema: {0} ({1})".format(
                cv_schema, good_values))
            sys.exit(1)
    ...
    if config.has_option("parameters", 'ordinal_attributes'):
        ordinal_attributes_list = config.get(
            "parameters", "ordinal_attributes")
...
def make_tarfile(path_results):

    with tarfile.open(path_results+"iterations.tar.gz", "w:gz") as tar:
        files = os.listdir(path_results)
        for folder in files:
            if folder.startswith("it"):
                tar.add(path_results+folder)

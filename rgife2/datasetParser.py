...
def get_index_selectedatts(selectedAtt_file,attributes_list):  ##8866 with path
    indexes = []
    for line in open(selectedAtt_file):
        if line.lower().startswith("@att"):
            line = line.strip("\n\r")
            att_name = line.split()[1]
            indexes.append(attributes_list.index(att_name))
    return indexes

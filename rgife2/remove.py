...
def remove_attributes(block_size, actual_iteration, reference_iteration, starting_index, white_list, path_results, name, n_folds, dataset_name, binarised_attribute_mapping, pre_processing):
    ...
    args = ["cp", path_results + "it"+str(reference_iteration)+"/"+name+"_it"+str(reference_iteration) +
            ".arff", path_results + "it"+str(actual_iteration)+"/"+name+"_it"+str(actual_iteration)+".arff"]
    subprocess.call(args)
    # copy the Training and the Test set from the previous iteration
    for fold in range(0, n_folds):
        subprocess.call("cp " + path_results + "it"+str(reference_iteration)+"/TrainFold" +
                        str(fold) + " " + path_results + "it"+str(actual_iteration)+"/", shell=True)
        subprocess.call("cp " + path_results + "it"+str(reference_iteration)+"/TestFold" +
                        str(fold) + " " + path_results + "it"+str(actual_iteration)+"/", shell=True)
    ...
    indexes = datasetParser.get_index_atts(
        path_results + "it"+str(actual_iteration)+"/TrainFold0")
    indexes_to_remove = []
    atts_to_remove = {}
    ...
    for fold in range(0, n_folds):
        filter_attributes_fold(path_results+"it"+str(actual_iteration)+"/", path_results+"it"+str(
            actual_iteration)+"/TrainFold"+str(fold), indexes_to_remove, atts_to_remove, actual_iteration)
        filter_attributes_fold(path_results+"it"+str(actual_iteration)+"/", path_results+"it"+str(
            actual_iteration)+"/TestFold"+str(fold), indexes_to_remove, atts_to_remove, actual_iteration)
    # remove files from the whole dataset
    if pre_processing == 0:
        filter_attributes_fold(path_results+"it"+str(actual_iteration)+"/", path_results + "it"+str(actual_iteration) +
                               "/"+name + "_it"+str(actual_iteration)+".arff", indexes_to_remove, atts_to_remove, actual_iteration)
    else:
        # check which binarised atts are kept and filter the original (no binarise atts) dataset according to them
        filter_whole_dataset(path_results+"it"+str(actual_iteration)+"/TrainFold0", path_results + "it"+str(actual_iteration) +
                             "/"+name + "_it"+str(actual_iteration)+".arff", actual_iteration, binarised_attribute_mapping, dataset_name)

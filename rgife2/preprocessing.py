                    ...
                    if attribute_type == "numeric" or attribute_type == "real" or attribute_type == "string" or attribute_name.lower() == "id":
                        ...
                        if len(values) > 2:
                            ...
                            for i, value in enumerate(values):
                                ...
                                binarised_attribute_mapping["is_{0}_{1}".format(
...
def preprocess_dataset(pre_processing, path_results, actual_iteration, dataset_name, n_folds, binarised_attribute_mapping, isCategorical, relation_name):
    ...
    subprocess.run(["mkdir", "-p", path_results + "it" +
                     str(actual_iteration) + "/OriginalFiles"])
    subprocess.run(["cp", dataset_name, path_results+"it" +
                     str(actual_iteration)+"/OriginalFiles/"])

    for fold in range(0, n_folds):
        subprocess.run(["cp", path_results+"it"+str(actual_iteration)+"/TrainFold" +
                         str(fold), path_results+"it"+str(actual_iteration)+"/OriginalFiles/"])
        subprocess.run(["cp", path_results+"it"+str(actual_iteration)+"/TestFold" +
                         str(fold), path_results+"it"+str(actual_iteration)+"/OriginalFiles/"])

    if pre_processing == 1 or pre_processing == 2:
        for fold in range(0, n_folds):
            compute_missing_values_mean(path_results+"it"+str(actual_iteration)+"/TrainFold"+str(
                fold), path_results+"it"+str(actual_iteration)+"/TestFold"+str(fold), isCategorical, relation_name)
        if pre_processing == 1:
            # missing values and cat.attributes
            for fold in range(0, n_folds):
                binarise_attributes(path_results+"it"+str(actual_iteration) +
                                    "/TrainFold"+str(fold)+"_NoMissing", binarised_attribute_mapping)
                binarise_attributes(path_results+"it"+str(actual_iteration) +
                                    "/TestFold"+str(fold)+"_NoMissing", binarised_attribute_mapping)
                subprocess.run(["mv",  path_results+"it"+str(actual_iteration)+"/TrainFold"+str(
                    fold)+"_NoMissing_Binarised", path_results+"it"+str(actual_iteration)+"/TrainFold"+str(fold)])
                subprocess.run(["mv",  path_results+"it"+str(actual_iteration)+"/TestFold"+str(
                    fold)+"_NoMissing_Binarised", path_results+"it"+str(actual_iteration)+"/TestFold"+str(fold)])
                subprocess.run(
                    ["rm",  path_results+"it"+str(actual_iteration)+"/TrainFold"+str(fold)+"_NoMissing"])
                subprocess.run(
                    ["rm",  path_results+"it"+str(actual_iteration)+"/TestFold"+str(fold)+"_NoMissing"])
        else:
            # only missing values
            for fold in range(0, n_folds):
                subprocess.run(["mv",  path_results+"it"+str(actual_iteration)+"/TrainFold"+str(
                    fold)+"_NoMissing", path_results+"it"+str(actual_iteration)+"/TrainFold"+str(fold)])
                subprocess.run(["mv",  path_results+"it"+str(actual_iteration)+"/TestFold"+str(
                    fold)+"_NoMissing", path_results+"it"+str(actual_iteration)+"/TestFold"+str(fold)])
    else:
        for fold in range(0, n_folds):
            binarise_attributes(path_results+"it"+str(actual_iteration) +
                                "/TrainFold"+str(fold), binarised_attribute_mapping)
            binarise_attributes(path_results+"it"+str(actual_iteration) +
                                "/TestFold"+str(fold), binarised_attribute_mapping)
            subprocess.run(["mv",  path_results+"it"+str(actual_iteration)+"/TrainFold"+str(
                fold)+"_Binarised", path_results+"it"+str(actual_iteration)+"/TrainFold"+str(fold)])
            subprocess.run(["mv",  path_results+"it"+str(actual_iteration)+"/TestFold"+str(
                fold)+"_Binarised", path_results+"it"+str(actual_iteration)+"/TestFold"+str(fold)])
...
def compute_missing_values_mean(training_set, test_set, isCategorical, relation_name):
    ...
    for column in range(0, training_matrix.shape[1]):
        if "?" in training_matrix[:, column]:
            ...
            training_matrix[training_matrix[:, column]
                            == "?", column] = imputed
